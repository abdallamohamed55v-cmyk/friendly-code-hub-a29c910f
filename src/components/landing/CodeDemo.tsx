...
const conversation: {
  ...
  {
    role: "user",
    content: "Build me a music streaming website with a modern dark theme",
    delay: 800,
  },
  {
    role: "assistant",
    content:
      "On it! I'm creating a music streaming app with:\n\n• Dark glassmorphism UI\n• Artist cards with hover effects\n• Music player bar with controls\n• Playlist sidebar\n\nGenerating code now...",
    delay: 400,
  },
  ...
  { role: "user", content: "Add a search bar with filters for genre and artist", delay: 2800 },
  {
    role: "assistant",
    content:
      "Done! Added a search bar with real-time filtering, genre chips (Pop, Rock, Hip-Hop, Jazz, Electronic), and artist suggestions dropdown. Try it out in the preview!",
    delay: 400,
  },
...
const CodeDemo = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setMessages((prev) =>
        prev.map((m, i) =>
          i === prev.length - 1
            ? { ...m, displayedContent: fullContent.slice(0, currentLen + CHARS_PER_TICK) }
            : m,
        ),
      );
    }, CHAR_SPEED);
  ...
  return (
    <div
      ref={rootRef}
      className="flex h-full flex-col overflow-hidden rounded-2xl border border-border/30 bg-card/30 backdrop-blur-md"
    >
      <div
        ref={scrollRef}
