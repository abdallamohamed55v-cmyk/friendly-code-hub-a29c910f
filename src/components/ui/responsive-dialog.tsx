...
export function ResponsiveDialogContent({ children, className, desktopClassName }: ContentProps) {
  const isMobile = useIsMobile();
  if (isMobile) {
...
export function ResponsiveDialogTitle({
  ...
  return (
    <DialogTitle
      className={cn("text-base font-semibold text-foreground tracking-tight text-left", className)}
    >
      {children}
    </DialogTitle>
  );
...
export function ResponsiveDialogDescription({
  ...
  const isMobile = useIsMobile();
  if (isMobile) {
    return <GlassSheetDescription className={className}>{children}</GlassSheetDescription>;
  }
  return (
    <DialogDescription className={cn("text-xs text-muted-foreground text-left mt-0.5", className)}>
      {children}
    </DialogDescription>
  );
}
