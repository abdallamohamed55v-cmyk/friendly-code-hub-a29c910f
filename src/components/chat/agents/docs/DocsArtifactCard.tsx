...
export default function DocsArtifactCard({ artifactId, title, docType, html: inlineHtml }: Props) {
  ...
  const openFull = () => {
    if (!isStreaming && html) setOpen(true);
  };
        ...
        >
          {html ? (
            <>
              ...
              {/* Hover "Open" badge */}
              {!isStreaming && (
                <div className="absolute top-3 right-3 inline-flex items-center gap-1 rounded-full bg-black/50 backdrop-blur px-2.5 py-1 text-[11px] font-medium text-foreground opacity-0 group-hover:opacity-100 transition">
