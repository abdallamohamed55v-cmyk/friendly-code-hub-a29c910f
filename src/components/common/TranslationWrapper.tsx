...
const TranslationWrapper = ({ children }: TranslationWrapperProps) => {
  ...
  useEffect(() => {
    ...
    const reTranslate = () => {
      ...
      // Re-set cookie so Google Translate re-walks newly mounted React subtrees
      // for the new route. We DO NOT toggle the select to "en" and back —
      // that round-trip causes a visible flash on every SPA navigation that
      // feels like a full page refresh.
      setGoogTransCookie(LANG_MAP[lang] || lang);
      const select = document.querySelector<HTMLSelectElement>(".goog-te-combo");
      if (select) {
        // Dispatch a change in-place; if it's already on the target value,
        // Google Translate will simply re-scan the DOM without flipping back to English.
        select.value = LANG_MAP[lang] || lang;
        select.dispatchEvent(new Event("change"));
        setTimeout(translateInputAttributes, 1000);
      } else {
        triggerTranslateOnce(lang);
      }
    };
