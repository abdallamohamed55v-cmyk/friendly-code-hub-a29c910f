...
import { createJob, JobWriter, loadJobForResume, runInBackground } from "../_shared/jobs.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Internal RESUME path (watchdog re-dispatch) ──
    // Re-hydrate user + input from the saved job row and reuse the same job id.
    let user: { id: string; email?: string } | null = null;
    let reuseJobId: string | undefined;
    if (isInternalCaller(req) && body?.action === "resume" && body?.jobId) {
      const job = await loadJobForResume(String(body.jobId));
      ...
      if (job.kind !== "docs") return new Response(JSON.stringify({ error: "wrong_runner" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const inp = (job.input ?? {}) as Record<string, unknown>;
      user = { id: job.user_id };
      body = {
        prompt: inp.prompt,
        history: inp.history ?? [],
        clarifications: inp.clarifications,
